...
Scheduler.plugin(function(e) {
    e._temp_matrix_scope = function() {
        e.matrix = {}, e._merge = function(e, t) {
            ...
            var _ = e._border_box_bvents();
            _ && (o += 3);
            var l = e._get_timeline_event_y(t._sorder, d),
                c = d + l + 2;
            ...
            h = "dhx_cal_event_line " + (h || ""), e.getState().select_id == t.id && (h += " dhx_cal_event_selected"), t._no_drag_move && (h += " no_drag_move");
            var u = t.color ? "background:" + t.color + ";" : "",
                v = t.textColor ? "color:" + t.textColor + ";" : "",
                f = e.templates.event_bar_text(t.start_date, t.end_date, t),
                g = "<div " + e._waiAria.eventBarAttrString(t) + " event_id='" + t.id + "' class='" + h + "' style='" + u + v + "position:absolute; top:" + l + "px; height: " + o + "px; left:" + r + "px; width:" + Math.max(0, s - r) + "px;" + (t._text_style || "") + "'>";
            if (e.config.drag_resize && !e.config.readonly) {
                var m = "dhx_event_resize",
                    p = "<div class='" + m + " " + m + "_start' style='height: " + o + "px;'></div>",
                    y = "<div class='" + m + " " + m + "_end' style='height: " + o + "px;'></div>";
                g += (t._no_resize_start ? "" : p) + (t._no_resize_end ? "" : y)
            }
            if (g += f + "</div>", !a) return g;
            var x = document.createElement("div");
            x.innerHTML = g;
            var b = this.order[i],
                w = e._els.dhx_cal_data[0].firstChild.rows[b];
            if (w) {
                var k = w.cells[1].firstChild;
                e._rendered.push(x.firstChild), k.appendChild(x.firstChild)
            }
            ...
            if (_ > 0) {
                ...
                s ? +o > +h && a && (d = e._cols[l]) : (_ = o - h, i.first_hour || i.last_hour ? (_ -= i._start_correction, 0 > _ && (_ = 0), d = Math.round(_ / r), d > e._cols[l] && (d = e._cols[l])) : d = Math.round(_ / r))
            }
            var u = e._border_box_bvents();
            return n += a ? 0 === _ || s ? d - (u ? 2 : 14) : d - (u ? 0 : 12) : d + 1
        }, e._timeline_get_rounded_date = function(t, a) {
            var i = e._get_date_index(this, t),
                n = this._trace_x[i];
            return a && +t != +this._trace_x[i] && (n = this._trace_x[i + 1] ? this._trace_x[i + 1] : e.date.add(this._trace_x[i], this.x_step, this.x_unit)),
                new Date(n)
            ...
            if (t && "cell" != this.render) {
                ...
                for (var p = 0; n > p; p++) a += e.render_timeline_event.call(this, t[p], !1)
            }
            return a
            ...
            for (var r = 0; r < this.y_unit.length; r++) {
                ...
                e._merge(l, {
                        height: this.dy
                    }), this.section_autoheight && (this.y_unit.length * l.height < t.offsetHeight && (l.height = Math.max(l.height, Math.floor((t.offsetHeight - 1) / this.y_unit.length))), this._section_height[this.y_unit[r].key] = l.height), l.td_className || (l.td_className = "dhx_matrix_scell" + (e.templates[this.name + "_scaley_class"](this.y_unit[r].key, this.y_unit[r].label, this.y_unit[r]) ? " " + e.templates[this.name + "_scaley_class"](this.y_unit[r].key, this.y_unit[r].label, this.y_unit[r]) : "")),
                    l.td_content || (l.td_content = e.templates[this.name + "_scale_label"](this.y_unit[r].key, this.y_unit[r].label, this.y_unit[r])), e._merge(l, {
                        tr_className: "",
            ...
            for (var v = t.firstChild.rows, f = null, r = 0, g = _.length; g > r; r++) {
                ...
                var m = f.key,
                    p = this._scales[m] = e._isRender("cell") ? v[r] : v[r].childNodes[1].getElementsByTagName("div")[0];
                e.callEvent("onScaleAdd", [p, m])
            }
            ...
            if (t.innerHTML = "<div></div>", this.second_scale) {
                for (var u = this.second_scale.x_unit, v = [this._trace_x[0]], f = [], g = [this.dx, this.dx], m = 0, p = 0; p < this._trace_x.length; p++) {
                    var y = this._trace_x[p],
                        x = e._timeline_is_new_interval(u, y, v[m]);
                    x && (++m, v[m] = y, g[m + 1] = g[m]);
                    var b = m + 1;
                    f[m] = e._cols[p] + (f[m] || 0), g[b] += e._cols[p]
                }
                ...
                for (var E = 0; E < v.length; E++) {
                    ...
                    S.className = "dhx_scale_bar dhx_second_scale_bar" + (N ? " " + N : ""), e.set_xy(S, f[E] - 1, i - 3, g[E], 0),
                        S.innerHTML = e.templates[this.name + "_second_scale_date"](D), w.appendChild(S)
                }
            }
            ...
            if ("cell" == t.render) {
                ...
                for (var d = "", o = 0; o < r.length; o++) {
                    ...
                    d += "<div class='dhx_tooltip_line' event_id='" + r[o].id + "' style='" + _ + l + "'>", d += "<div class='dhx_tooltip_date'>" + (r[o]._timed ? e.templates.event_date(r[o].start_date) : "") + "</div>", d += "<div class='dhx_event_icon icon_details'>&nbsp;</div>", d += e.templates[t.name + "_tooltip"](r[o].start_date, r[o].end_date, r[o]) + "</div>"
                }
                e._tooltip.style.display = "", e._tooltip.style.top = "0px", document.body.offsetWidth - i.left - e._tooltip.offsetWidth < 0 ? e._tooltip.style.left = i.left - e._tooltip.offsetWidth + "px" : e._tooltip.style.left = i.left + a.src.offsetWidth + "px",
                    e._tooltip.date = n, e._tooltip.innerHTML = d, document.body.offsetHeight - i.top - e._tooltip.offsetHeight < 0 ? e._tooltip.style.top = i.top - e._tooltip.offsetHeight + a.src.offsetHeight + "px" : e._tooltip.style.top = i.top + "px"
            }
            ...
            if (a && "cell" == a.render) {
                if (a) {
                    ...
                    if (i) return e._timeline_showToolTip(a, i, e.$domHelpers.getOffset(i.src))
                }
                e._timeline_hideToolTip()
            }
            ...
            if (e.matrix && e.matrix[e._mode]) {
                ...
                if (n) l = [a], _ = [n];
                    ...
                    if (c) h.hasOwnProperty(c) && (_.push(c), l.push(d._scales[c]));
                        ...
                        for (var u in h) h.hasOwnProperty(u) && (_.push(u), l.push(d._scales[u]))
                }
                ...
                for (var m = t.zones, p = e._get_css_classes_by_config(t), y = 0; y < _.length; y++) {
                    a = l[y], n = _[y];
                    for (var v = 0; v < f.length; v++)
                        for (var x = f[v], b = 0; b < m.length; b += 2) {
                            var w = m[b],
                                k = m[b + 1],
                                E = new Date(+x + 60 * w * 1e3),
                                D = new Date(+x + 60 * k * 1e3);
                            if (E = new Date(E.valueOf() + 1e3 * (E.getTimezoneOffset() - x.getTimezoneOffset()) * 60),
                                D = new Date(D.valueOf() + 1e3 * (D.getTimezoneOffset() - x.getTimezoneOffset()) * 60), D > r && s > E) {
                                ...
                                N.style.cssText = "height: " + A + "px; left: " + S + "px; width: " + M + "px; top: 0;", a.insertBefore(N, a.firstChild), o.push(N)
                            }
                        }
                }
                return o
        ...
        e._on_scale_add_marker = function(t, a) {
                ...
                if (n && e.matrix && e.matrix[e._mode])
                    for (var r = e._mode, d = e._min_date, o = e._max_date, _ = n.global, l = e.date.date_part(new Date(d)); o > l; l = e.date.add(l, 1, "day")) {
                        ...
                        for (var m = 0; m < u.length; m++) {
                            var p = u[m],
                                y = p.days;
                            7 > y ? (y = c, i.push.apply(i, e._render_marked_timespan(p, t, a, l, e.date.add(l, 1, "day"))), y = h) : i.push.apply(i, e._render_marked_timespan(p, t, a, l, e.date.add(l, 1, "day")))
                        }
                    }
                return i
            }
            ...
            var c;
            if (o >= r._trace_x.length) c = e.date.add(r._trace_x[r._trace_x.length - 1], r.x_step, r.x_unit), r._end_correction && (c = new Date(c - r._end_correction));
            else {
